...
**The choice.** MCP boot no longer spawns chromium. The LLM calls `browser_open` to start (spawn or attach) and `browser_close` to tear down. Profile behaviour is selected by environment: ephemeral by default, persistent via `VIMX_PROFILE_DIR`, clone-on-spawn via `VIMX_PROFILE_TEMPLATE`, or external-attach via `CDP_TARGET` / `CDP_PORT`. The five branches live in `cdp.ts:spawnOrAttach` and are documented inline at `cdp.ts:115-139`.
...
|---|---|---|
| **attach** | `CDP_TARGET` or `CDP_PORT` set | Dev workflow (`scripts/dev-chrome.sh`) and per-worktree dedicated chromium driven by cwt hooks. vimx doesn't kill what it didn't spawn. |
| **template clone** | `VIMX_PROFILE_TEMPLATE=/path` | Browser-per-agent **with consistent boot state**. Two agents (two MCP servers) each clone the template to their own `/tmp/vimx-mcp-*`, so both start signed into Google without fighting over chromium's per-user-data-dir singleton lock. |
| **persistent** | `VIMX_PROFILE_DIR=/path` | Cookies survive MCP restarts. Required for sites like Google that flag fresh-profile sessions as suspected automation. Single-share only — cannot be used by two MCP servers concurrently; an existing chromium on that dir is *attached*, not re-spawned. |
| **ephemeral** | neither env set | Default. Fresh `/tmp/vimx-mcp-*` dir per `browser_open`, wiped on `browser_close`. Right for stateless one-off automation. |

`VIMX_PROFILE_TEMPLATE` overrides `VIMX_PROFILE_DIR` if both are set — clone semantics beat single-share.

**Crash recovery.** SIGKILLed MCP servers can't run cleanup paths. Each dir we create writes a `vimx.pid` file; on the next MCP boot, `sweepStaleProfiles` (cdp.ts:162) walks `/tmp/vimx-mcp-*` once, and for each dir whose owner pid is dead it: looks up any orphan chromium via `SingletonLock` → `/proc/<pid>/cmdline` verification, kills it, then wipes the dir. The `/proc/cmdline` check prevents pid-recycle disasters (a recycled pid that's now an unrelated process won't get SIGKILLed).
...
**Architectural framing.** This is the same shift as the earlier "agent owns task semantics, tool owns mechanics" line — applied to process lifetime. The agent's affordances now include *having a body* and *not having one*; vimx stops pretending the chromium is part of the MCP server itself. The profile-mode env vars are configuration of *which body, with what state*, not behaviour the agent has to think about per-call.
...
- Commits, in order: `9f6f7a6` (auto-spawn per MCP server), `17deedf` (explicit `browser_open`/`browser_close`), `2a1043e` (`VIMX_PROFILE_DIR`), `ff739fc` (`VIMX_PROFILE_TEMPLATE`), `6c8d70f` (stale-dir sweep), `49d66d3` (attach-liveness fix).
