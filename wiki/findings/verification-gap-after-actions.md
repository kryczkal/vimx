...
- webpilot already surfaces `Value now: "..."` after `type()` and explicit state after `toggle()` / `select()`. The gap is `press()` — the highest-frequency action, and the one whose effects are most variable.
