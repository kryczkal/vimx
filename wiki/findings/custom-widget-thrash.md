...
**Claim.** Custom dropdowns / menus / comboboxes (the ones that aren't native `<select>` elements) are the single most expensive failure pattern in webpilot sessions. The scan output doesn't distinguish "real button" from "menu trigger that opens with Enter/Space" — so the agent tries every tool in sequence before stumbling onto keyboard navigation.
...
- Across the 10 webpilot-dominant sessions, every long session contains at least one widget-thrash episode.

**Implication.** Webpilot can't currently tell the agent "this is a custom dropdown that opens with Enter/Space and navigates with arrows." Two complementary fixes:
